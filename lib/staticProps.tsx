...
async function getI18nTexts(locale: string) {
	...
	fullTerms.forEach(
		(term: { term: string; translation: { content: string } }) => {
...
async function getPrefixConversions() {
	if (backendCache.get('prefixConversions'))
		return backendCache.get('prefixConversions')
	const client = await getStandaloneApolloClient()
...
export const getCommonStaticProps: GetCommonStaticProps = async (
...
type GetPageStaticProps = (
	context: GetStaticPropsContext,
	staticSlug?: string
) => Promise<any>

export const getPageStaticProps: GetPageStaticProps = async (
	context,
	staticSlug
) => {
	...
	let endpoint = 'pages'
	if (staticSlug === '/') slug = '/'
	if (staticSlug === '/country' || staticSlug === '/project') {
		slug = ''
	}
	...
	if(slug) {
		...
		if (!pages) {
			...
			api = await fetch(
				`${process.env.NEXT_PUBLIC_CMS_URL}/api/${endpoint}?locale=${context.locale}`,
				{headers}
			)
			if (!api.ok) {
				console.log('getPageStaticProps Fetch ', endpoint, api.status, api.statusText)
		...
		if (!p) {
			console.log('getPageStaticProps 404 ')
			return {notFound: true, revalidate}
		}

		api = await fetch(
			`${process.env.NEXT_PUBLIC_CMS_URL}/api/${endpoint}/${p.id}?locale=${context.locale}&populate=deep`,
			{headers}
		)
		...
		if (!api.ok) {
			if (api.status === 404) {
				console.log(
					`! getPageStaticProps 404 due to API 404: /api/${endpoint}/${p.id}?locale=${context.locale}&populate=deep`
				)
				return {notFound: true, revalidate}
			}
			throw new Error(`Page fetch failed: ${api.status} ${api.statusText}`)
	...
	let menu = backendCache.get(`menu-${locale}`)
	if (!menu) {
		api = await fetch(
			`${process.env.NEXT_PUBLIC_CMS_URL}/api/menu?populate=deep`,
			{headers}
		)
		...
		backendCache.set(`menu-${locale}`, menu, cacheTTL)
	}

	let footer = backendCache.get(`footer-${locale}`)
	if (!footer) {
		api = await fetch(
			`${process.env.NEXT_PUBLIC_CMS_URL}/api/footer?populate=deep`,
			{headers}
		)
		...
		backendCache.set(`footer-${locale}`, footer, cacheTTL)
	}
