...
describe('GAS', () => {
    describe('Combustion scope3', () => {
        it(' Gas energy', () => {
            ...
            const result = isoGasEnergy.unwrap(
                calculateGasEnergy(gasProduction)(
        ...
        it(' Gas CO2E Combustion emissions', () => {
            ...
            const result = isoGasCO2ECombustionEmissions.unwrap(
                calculateGasCO2ECombustionEmissions(gasEnergy)(
    ...
    describe('Production scope1', () => {
        it('Should calculate barrels of oil equivalent', () => {
            ...
            const boePere6m3 = isoBOEPere6m3.wrap(5.883)
            const gasProduction = isoGasProduction.wrap(100)
        ...
        it('Gas CO2 Production emissions', () => {
            const barrelsOfOilEquivalent =
                isoBarrelsOfOilEquivalent.wrap(588300000)
            ...
            expect(result.p5.toPrecision(10)).toBe(
                expectedResult.p5.toPrecision(10)
            )
            expect(result.wa.toPrecision(10)).toBe(
                expectedResult.wa.toPrecision(10)
            )
            expect(result.p95.toPrecision(10)).toBe(
                expectedResult.p95.toPrecision(10)
            )
        })
    })
    describe('CH4', () => {
    ...
    it(' Upstream Total ', () => {
        ...
        const result = isoGasCO2EProductionEmissions.unwrap(
            calculateGasCO2EProductionEmissions(CO2ProductionEmissions)(
                CO2EOfMethane
            )
        )
