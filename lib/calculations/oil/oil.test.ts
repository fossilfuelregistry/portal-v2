...
describe('Oil', () => {
    describe('Combustion emissions', () => {
        it('Should calculate the energy from volume', () => {
            ...
            expect(result.p5.toFixed(0)).toBe(
                expectedEnergyOutcome.p5.toString()
            )
            expect(result.wa.toFixed(0)).toBe(
                expectedEnergyOutcome.wa.toString()
            )
            expect(result.p95.toFixed(0)).toBe(
                expectedEnergyOutcome.p95.toString()
            )
        })
        it(' 2. Energy -> Emissions ', () => {
            ...
            const result = isoOilCO2ECombustionEmissions.unwrap(
                calculateCombustionEmissions(energyFromOil)(
                    energyToEmissionsFactors
                )
            )
