...
import {
  GQL_projectProduction,
  GQL_projectProjection,
  GQL_projectReserves,
...
const useProjectData = ({
  ...
  const { co2FromVolume, reservesProduction, co2eFromVolume, projectCO2 } =
    useConversionHooks({
      conversionConstants,
      allSources,
      gwp,
  ...
  const _co2 = (dataset: MinimalDataset[]) => {
    try {
      return (dataset ?? [])
        .filter(
          (datapoint) =>
            datapoint.fossilFuelType === 'gas' ||
            datapoint.fossilFuelType === 'oil' ||
            datapoint.fossilFuelType === 'coal'
        )
        .map((datapoint) => ({
          ...datapoint,
          id: undefined,
  ...
  const production = useMemo(() => {
    DEBUG &&
      console.info(
        '_co2( productionData )',
        productionData?.projectDataPoints?.nodes
      )
    return _co2(productionData?.projectDataPoints?.nodes)
  }, [
    productionData?.projectDataPoints?.nodes,
    productionData?.projectDataPoints?.nodes?.length,
    productionSourceId,
    gwp,
  ])
  ...
  const projection = useMemo(() => {
    ...
    projectionData?.projectDataPoints?.nodes,
    projectionSourceId,
    stableProduction,
    gwp,
  ])
  ...
  const reserves = useMemo(
    () => _co2(reservesData?.projectDataPoints?.nodes),
  ...
  useEffect(() => {
    ...
    const newLimits = production.reduce(
      (_limits, datapoint) => {
