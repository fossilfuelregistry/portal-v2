...
async function getCalculationConstants() {
	if( myCache.get( 'calculationConstants' ) ) return myCache.get( 'calculationConstants' )
	const client = await getStandaloneApolloClient()
...
export const getCommonStaticProps = async( context ) => {
	try {
		...
		return { props: {texts, conversions, sources, locale, CO2Costs, constants, prefixConversions }}
	}
	catch( e ) {
